/* empty css                      */import{a}from"./assets/vendor-a61d8330.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const c of r.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&i(c)}).observe(document,{childList:!0,subtree:!0});function o(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerpolicy&&(r.referrerPolicy=s.referrerpolicy),s.crossorigin==="use-credentials"?r.credentials="include":s.crossorigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(s){if(s.ep)return;s.ep=!0;const r=o(s);fetch(s.href,r)}})();class T{constructor(){}saveInStorage(e,o){localStorage.setItem(e,JSON.stringify(o))}saveAllToLocalStorage(e){localStorage.setItem("products",JSON.stringify(e))}saveCategories(){if(!localStorage.getItem("categories"))return n.getCategories().then(e=>{console.log(e),localStorage.setItem("categories",JSON.stringify(e))})}defaultApiOptions(){const e={keyword:null,category:null,page:1,limit:6};localStorage.setItem("options",JSON.stringify(e))}getApiOptions(){return JSON.parse(localStorage.getItem("options"))}setApiOptions(e,o){JSON.parse(localStorage.getItem("options")),options[e]=o}addToCart(e){const i=JSON.parse(localStorage.getItem("products")).find(s=>s._id===e);if(console.log(i),localStorage.getItem("cart")){const s=JSON.parse(localStorage.getItem("cart"));return localStorage.setItem("cart",JSON.stringify(s))}else return localStorage.setItem("cart",JSON.stringify(i))}getFromStorage(e){return JSON.parse(localStorage.getItem(`${e}`))}removeFromCart(e){localStorage.getItem("products"),JSON.parce(localStorage.getItem("cart"))}clearCart(){}saveOptionsToFoodService(e){n.perPage=e.limit,n.currentPage=e.page,n.category=e.category,n.searchQuerry=e.keyword}}class k{constructor(){this.URL="https://food-boutique.b.goit.study/api",this.currentPage=1,this.perPage=6,this.searchQuerry="",this.category=""}getBasicFoodList(){return window.innerWidth>=768&&window.innerWidth<1440?this.perPage=8:window.innerWidth>=1440&&(this.perPage=9),a.get(`${this.URL}/products?limit=${this.perPage}`).then(e=>e.data)}getFoodListWithOptions(){return window.innerWidth>=768&&window.innerWidth<1440?this.perPage=8:window.innerWidth>=1440&&(this.perPage=9),a.get(`${this.URL}/products?page=${this.currentPage}&limit=${this.perPage}&keyword=${this.searchQuerry}&category=${this.category}`).then(e=>e.data)}findProductById(e){return a.get(`${this.URL}/products/${e}`).then(o=>o.data)}getPopular(){return a.get(`${this.URL}/products/popular`).then(e=>e.data)}getDiscount(){return a.get(`${this.URL}/products/discount`).then(e=>e.data)}getCategories(){return a.get(`${this.URL}/products/categories`).then(e=>e.data)}resetPageCounter(){this.currentPage=1}incrementPage(){this.currentPage+=1}resetSearchQuerry(){this.searchQuerry=""}resetCategory(){this.category=""}subscribe(e){a.post(`${this.URL}/subscription`,{email:e})}order(e,o){a.post(`${this.URL}/orders`,{email:e,products:o})}}const O="/project-code_busters09/assets/cart-a9135e8e.svg",S="/project-code_busters09/assets/cartLight-0b4d3d6f.svg",y="/project-code_busters09/assets/discount-b955391f.svg",b=document.querySelector(".filter-box__list"),N=document.querySelector(".popular-products__list"),A=document.querySelector(".discount-products__list"),H=document.querySelector(".popular-products__list-responsive"),R=document.querySelector(".discount-products__list-responsive"),p=new T,n=new k;document.addEventListener("click",function(t){if(t.target.classList.contains("cart-img-products")){const e=t.target,o="../img/checked.svg";e.src=o}});v();q();J();function v(){n.getBasicFoodList().then(t=>{console.log(t),b.innerHTML=P(t.results),p.saveAllToLocalStorage(t.results),p.defaultApiOptions()}).catch(t=>{})}function m(){const t=p.getApiOptions();n.getFoodListWithOptions(t).then(e=>{b.innerHTML=P(e.results),p.saveAllToLocalStorage(e.results)}).catch(e=>{})}function q(){n.getPopular().then(t=>{N.innerHTML=$(t),H.innerHTML=$(t)}).catch(t=>{})}function J(){n.getDiscount().then(t=>{A.innerHTML=w(t),R.innerHTML=w(t)}).catch(t=>{})}function P(t){return t.map(({_id:e,name:o,img:i,category:s,price:r,popularity:c,size:f,is10PercentOff:g})=>{const l=document.createElement("img");l.src=O,l.classList.add("main-cart-icon");const d=document.createElement("img");d.src=y,d.classList.add("discount-icon-products");const h=g?`${d.outerHTML}`:"";return`<li class="product-card" data-id=${e}>
    ...
    </li>`}).slice(0,2).join("")}window.addEventListener("resize",U);function U(){const t=window.innerWidth;t>=768&&t<1440&&n.perPage!==8?(n.perPage=8,m()):t>=1440&&n.perPage!==9?(n.perPage=9,m()):t<768&&n.perPage!==6&&(n.perPage=6,m())}const E=document.querySelector(".submit-form"),u=document.querySelector(".categories"),_=document.querySelector(".filter-box__list");document.addEventListener("DOMContentLoaded",function(){E.elements.search.value="",u.value=""});E.addEventListener("submit",t=>{t.preventDefault();const e=t.target.elements.search.value;n.searchQuerry=e;const o=p.getApiOptions();o.keyword=e||null,localStorage.setItem("options",JSON.stringify(o)),e?m():(n.resetSearchQuerry(),v())});u.addEventListener("change",function(){const t=u.value,e=p.getApiOptions();e.category=t||null,localStorage.setItem("options",JSON.stringify(e)),u.value==="show-all"?(_.innerHTML="",v()):(n.category=t,_.innerHTML="",m())});n.getCategories().then(t=>{const e=document.createElement("option");e.value="",e.textContent="Categories",e.disabled=!0,e.selected=!0;const o=t.map(s=>{const r=document.createElement("option");return r.value=s,r.textContent=s,r}),i=F();o.push(i),u.appendChild(e),u.append(...o)});function F(){const t=document.createElement("option");return t.textContent="Show All",t.value="show-all",t}const z={pagesRibbonEL:document.querySelector(".pag-ribbon")};z.pagesRibbonEL.addEventListener("click",D);function D(t){const e=t.target.closest("button").id;console.log(e)}const I=document.querySelector(".modal"),W=document.querySelector(".modal-content"),x=document.querySelector(".main-products");x.addEventListener("click",B);async function B(t){if(t.target.classList.contains("main-cart-icon"))return;const e=t.target.closest("li");if(e){const o=await n.findProductById(e.dataset.id);W.innerHTML=Q(o)}I.style.display="block",document.body.classList.add("no-scroll"),document.addEventListener("keydown",C),window.addEventListener("click",M)}function Q(t){return`
...
`}function C(t){t.key==="Escape"&&L()}function M(t){(t.target.classList.contains("modal")||t.target.closest(".modal-btn-close"))&&L()}function L(){I.style.display="none",document.removeEventListener("keydown",C),window.removeEventListener("click",M),document.body.classList.remove("no-scroll")}
